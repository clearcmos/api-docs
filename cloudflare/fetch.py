...
"""
Cloudflare API Documentation Fetcher

Fetches the official Cloudflare OpenAPI spec from GitHub and converts it
...
def fetch_url(url: str, timeout: int = 60) -> str | None:
    req = Request(url, headers={"User-Agent": "cloudflare-api-docs-fetcher/1.0"})
    try:
...
def sync(args: argparse.Namespace) -> None:
    ...
    print("Fetching Cloudflare OpenAPI spec...")
    ...
    main_lines = ["# Cloudflare API Documentation\n"]
    info = spec.get("info", {})
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch Cloudflare API docs from OpenAPI spec and convert to markdown"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
