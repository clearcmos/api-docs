...
"""
CircleCI API Documentation Fetcher

Fetches the official CircleCI OpenAPI spec from circleci.com and converts it
...
OPENAPI_URL = "https://circleci.com/api/v2/openapi.json"
...
def fetch_url(url: str, timeout: int = 60) -> str | None:
    req = Request(url, headers={"User-Agent": "circleci-api-docs-fetcher/1.0"})
    try:
...
def schema_to_markdown(schema: dict, spec: dict, depth: int = 0, seen: set | None = None) -> str:
    ...
    # Handle OpenAPI 3.1 list-type type fields (e.g. ["string", "null"])
...
def sync(args: argparse.Namespace) -> None:
    ...
    print("Fetching CircleCI OpenAPI spec...")
    ...
    main_lines = ["# CircleCI API Documentation\n"]
    info = spec.get("info", {})
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch CircleCI API docs from OpenAPI spec and convert to markdown"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
