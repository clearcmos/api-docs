...
- `terraform` accepts `--provider org/name` (interactive picker if omitted with a TTY). Each provider's docs are written under `docs/{org}/{name}/` so multiple providers coexist in one checkout; cache keys are prefixed with `{org}/{name}|` so reconciling one provider never touches another's files.
...
9. **Next.js page data** -- some Next.js doc sites serve MDX pages from a catch-all route and expose each page's props at `/_next/data/{buildId}{path}.json`, including the compiled MDX. Read the `buildId` from any page's `__NEXT_DATA__` first. Spotify uses this pattern.
10. **Authenticated APIs** -- some vendors require auth to access their API spec (e.g. SentinelOne management console). Accept credentials via CLI flags, never hardcode tenant URLs.
11. **HTML scraping** -- last resort. Some sites serve markdown alternates via `<link rel="alternate" type="text/markdown">`.
...
- Terraform: provider version plus docs-manifest hash plus the fetcher hash, keyed per provider in `.cache.json`.
...
- Spotify: spec ETag plus the fetcher hash for the reference tree, per-page ETags for the guides.
...
The one exception is YAML-only specs. If a vendor only publishes YAML (no JSON alternative), `pyyaml` is acceptable. Current examples: clickup (V3 spec), langfuse, okta, oracle, spotify.
...
- **terraform** -- Terraform Registry API example. The registry serves no ETag or Last-Modified, so the fast path is a per-provider source fingerprint in `.cache.json` under `source:{org}/{name}`: provider version + a hash of the docs manifest (immutable doc IDs plus output-relevant metadata) + the fetcher hash, with the outputs the last full run produced. When all of it matches and every recorded output exists, the fetcher skips all per-document requests -- and keying it per provider means alternating providers does not invalidate each other's fast path. The fingerprint is recorded only after every document request succeeded, so a partial run retries instead of qualifying. Changed providers fetch document bodies with 16 threads (measured on integrations/github, 164 docs: 8 workers ~4.0s, 16 ~2.7s, zero failures). A doc whose body fetch fails keeps its last known-good file, cache entry, and README line.
...
- **spotify** -- dual-source example (official OpenAPI YAML spec plus Next.js compiled-MDX guide pages), and the reference for the compiled-MDX pattern. `robots.txt` disallows `/reference/*.yaml` except `/reference/web-api/open-api-schema.yaml`, which is the authoritative spec (96 operations, 15 tags, 48 of them deprecated by the February 2026 changes); YAML-only, hence `pyyaml`. **Guides:** developer.spotify.com is a Next.js site with a `/documentation/[...mdx]` catch-all route, no sitemap and an llms.txt that lists only the spec plus one page. Page bodies come from `/_next/data/{buildId}/documentation/web-api/{path}.json?mdx=web-api&mdx=...` (one query param per path segment), whose `pageProps.source.compiledSource` is the MDX compiler's output. That compiled JS is the source rather than the rendered HTML because every class name on the rendered page is a styled-components hash that changes each build. The `buildId` is read from `__NEXT_DATA__` on the landing page, which also supplies the nav link list; a bounded BFS over `href: "/documentation/web-api..."` strings in the fetched sources catches any page missing from the nav (currently none). Note `/documentation/web-api/references/changes/*` are guides, not endpoints, so reference filtering must compare against `/reference` exactly, not by string prefix. **Compiled MDX to markdown:** `JsExpr` is a recursive-descent parser for the small JS subset that output uses (string/number/bool literals, arrays, object literals, dotted identifiers, `_jsx`/`_jsxs` calls), producing an element tree that preserves the original markdown structure exactly; `MdxRenderer` then walks it with the usual block/inline split. Vendor-specific components: Code Hike `CH.Code` carries syntax-highlighted token streams (`files[].code.lines[].tokens[].content` joined per line, language from `code.lang`), `CH.Section` wraps prose whose `CH.SectionLink` focus links flatten to plain text and whose `CH.SectionCode` placeholder emits the section's code block (hence a section-files stack), `Banner` becomes a GitHub alert keyed off its `color` prop, and the injected `<style dangerouslySetInnerHTML>` theme block is dropped. Unhandled component names are counted and reported in the run summary. A conversion failure preserves that page's previous output instead of dropping it. **Layout:** guides mirror their URL path (`concepts/scopes.md`, `references/changes/july-2026.md`) with the landing page at `docs/index.md`; endpoints live at `docs/reference/{tag}/{operationId}.md`, deviating from the usual `{method}-{path}.md` because Spotify's operationIds are exactly the vendor doc slugs, so a pasted `/documentation/web-api/reference/get-an-album` URL maps straight to a file. Titles and section grouping for the READMEs are cached per page, so a 304 does not need the body. `x-spotify-policy-list` refs point at an external `../policies.yaml` that robots.txt disallows; the policy titles/descriptions/URLs come instead from `pageProps.policyReferences`, which every guide page carries. `flatten_all_of` merges nested `allOf` chains recursively (Spotify nests paging objects one level down, so a single-level merge silently loses every property). **Caching:** the spec ETag plus the fetcher hash form the reference-tree fingerprint (all recorded outputs must still exist), and each guide page stores its own ETag plus fetcher hash, so a routine sync is 29 conditional requests and no bodies. A failed page or a failed spec fetch preserves the affected scope and suppresses removal detection for the run.
