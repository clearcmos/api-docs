...
"""
Discord Developer Documentation Fetcher

Fetches all human-readable pages under https://docs.discord.com/developers/
...
The sitemap is authoritative for what exists; llms.txt supplies readable
titles for category READMEs.
"""
...
SITE = "https://docs.discord.com"
DOCS_PREFIX = "/developers/"
...
def normalize_docs_url(url: str) -> str | None:
    ...
    Returns None if the URL is not under /developers/.
    """
    if not url.startswith(f"{SITE}{DOCS_PREFIX}"):
        return None
    url = url.rstrip("/")
    if url.endswith(".md"):
        url = url[:-3]
...
def path_for(url: str) -> tuple[str, str, str]:
    ...
    Single-segment URLs (e.g. /developers/intro) are top-level pages and
    get stored as _root/<name>.md so they don't collide with category
    directories. Multi-segment URLs go under <category>/.../ as .md, and
    ...
    if len(parts) == 1:
        category = "_root"
        subpath = rel
        file_path = os.path.join(DOCS_DIR, "_root", parts[0]) + ".md"
    else:
        category = parts[0]
        subpath = rel
...
def build_top_readme(categories: dict[str, list[dict]]) -> str:
    lines = ["# Discord Developer Documentation", ""]
    lines.append(f"*Mirrored from [{SITE}{DOCS_PREFIX}]({SITE}{DOCS_PREFIX}).*")
    lines.append("")
